...
const FRAMER_WEBHOOK_SECRET = process.env.FRAMER_WEBHOOK_SECRET;
 ...
 */
function verifyFramerSignature(body, signature, submissionId) {
  if (!FRAMER_WEBHOOK_SECRET) {
    console.warn('No webhook secret configured - skipping signature verification');
    return true;
  }

  const expectedSignature = crypto
    .createHmac('sha256', FRAMER_WEBHOOK_SECRET)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!verifyFramerSignature(body, signature, submissionId)) {
